...
public class h0 {
		...
		if (syntaxTree.isIdentifierNode(nodeIndex)) {
			...
			switch (attrReferenceKind) {
				...
				case 3 :
					if (syntaxTree.hasAttrType(nodeIndex) && syntaxTree.getAttrType(nodeIndex).isDelegateType()) {
						this.highlighterCallback.delegateFound(syntaxTree.getLanguage(),
								syntaxTree.getStartLine(nodeIndex), syntaxTree.getStartColumn(nodeIndex),
								syntaxTree.getEndLine(nodeIndex), syntaxTree.getEndColumn(nodeIndex));
					}
					break;
				case 6 :
					this.highlighterCallback.namespaceFound(syntaxTree.getLanguage(),
							syntaxTree.getStartLine(nodeIndex), syntaxTree.getStartColumn(nodeIndex),
							syntaxTree.getEndLine(nodeIndex), syntaxTree.getEndColumn(nodeIndex));
					break;
				case 7 :
				...
				case 14 :
				case 17 :
					this.highlighterCallback.typeFound(syntaxTree.getLanguage(), syntaxTree.getStartLine(nodeIndex),
							syntaxTree.getStartColumn(nodeIndex), syntaxTree.getEndLine(nodeIndex),
							syntaxTree.getEndColumn(nodeIndex));
					break;
				case 15 :
					this.highlighterCallback.identifierFound(syntaxTree.getLanguage(),
							syntaxTree.getStartLine(nodeIndex), syntaxTree.getStartColumn(nodeIndex),
							syntaxTree.getEndLine(nodeIndex), syntaxTree.getEndColumn(nodeIndex));
					break;
				case 16 :
			...
			switch (attrReferenceKind) {
				...
				case 25 :
					...
					break;
				case 26 :
					this.highlighterCallback.keywordFound(syntaxTree.getLanguage(), syntaxTree.getStartLine(nodeIndex),
							syntaxTree.getStartColumn(nodeIndex), syntaxTree.getEndLine(nodeIndex),
							syntaxTree.getEndColumn(nodeIndex));
					break;
				case 30 :
	...
	public void FH(FileEntry fileEntry) {
		if (fileEntry.getCodeModel() == null
				|| this.lexerHighlighterVersionMap.get(fileEntry.getId()) == fileEntry.getVersion()) {
			return;
		}
	...
	private void fillSemanticHighlighter_bak(SyntaxTree syntaxTree, int nodeIndex) {
		if (syntaxTree.isIdentifierNode(nodeIndex)) {
			...
			if (attrReferenceKind != 2 && attrReferenceKind != 3) {
				...
				if (attrReferenceKind != 30) {
					switch (attrReferenceKind) {
						...
						case 14 :
						case 17 :
							this.highlighterCallback.typeFound(syntaxTree.getLanguage(),
									syntaxTree.getStartLine(nodeIndex), syntaxTree.getStartColumn(nodeIndex),
									syntaxTree.getEndLine(nodeIndex), syntaxTree.getEndColumn(nodeIndex));
							break;
						case 15 :
							...
							break;
						case 16 :
							if (!syntaxTree.hasAttrType(nodeIndex)
									|| !syntaxTree.getAttrType(nodeIndex).isDelegateType()) {
								this.highlighterCallback.identifierFound(syntaxTree.getLanguage(),
										syntaxTree.getStartLine(nodeIndex), syntaxTree.getStartColumn(nodeIndex),
										syntaxTree.getEndLine(nodeIndex), syntaxTree.getEndColumn(nodeIndex));
								break;
					...
					switch (attrReferenceKind) {
						...
						case 25 :
							...
							break;
						case 26 :
							this.highlighterCallback.keywordFound(syntaxTree.getLanguage(),
									syntaxTree.getStartLine(nodeIndex), syntaxTree.getStartColumn(nodeIndex),
									syntaxTree.getEndLine(nodeIndex), syntaxTree.getEndColumn(nodeIndex));
							break;
					}
				...
				this.highlighterCallback.delegateFound(syntaxTree.getLanguage(), syntaxTree.getStartLine(nodeIndex),
						syntaxTree.getStartColumn(nodeIndex), syntaxTree.getEndLine(nodeIndex),
						syntaxTree.getEndColumn(nodeIndex));
			}
		}
