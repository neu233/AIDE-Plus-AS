    ...
    public boolean run() {
        ...
        if (ZeroAicySetting.isEnableEclipseJavaFormat()) {
            ...
            if ("java".equals(suffixName)) {
                ...
                AIDEEditor.t editorModel = AIDEEditorExtend.getEditorModel(currentEditor);
                if (editorModel == null) {
                    return true;
                }
        ...
        ServiceContainer.getEngineService().Hw(ServiceContainer.getOpenFileService().getVisibleFile(), startLine, endLine, ServiceContainer.getMainActivity().getAIDEEditorPager().getTabSize());
        return true;
    }
...
//									}
    ...
    public List<Modification> test(String filePath, String inputText, EditorModel editorModel, TextEdit edit) {
        ...
        for (TextEdit child : edit.getChildren()) {
            ...
            int startLine = startOffset >= 0
                    ? Util.getLineNumber(startOffset, lineEnds, 0, lineEnds.length - 1)
                    : 0;
            int startColumn = startOffset >= 0
                    ? Util.searchColumnNumber(lineEnds, startLine, startOffset)
                    : 0;

            int endLine = endOffset >= 0
                    ? Util.getLineNumber(endOffset, lineEnds, 0, lineEnds.length - 1)
                    : 0;

            int endColumn = endOffset >= 0
                    ? Util.searchColumnNumber(lineEnds, endLine, endOffset)
                    : 0;
            //
            endColumn += 1;
            ...
            // startColumn += 1;
            //*/
            Modification modification = new Modification(startLine, startColumn, endLine, endColumn, filePath, replaceEdit.getText(), false) {
            };
