...
public abstract class ResourceTable extends ChunkHeader {
    ...
    public static abstract class Component extends ChunkHeader {
        ...
        public Component(ResourceTable resourceTable, final short type) {
            super(type);
    ...
    public static abstract class Package extends Component {
        ...
        final List<TypeSpec> specs = new ArrayList<TypeSpec>();
        ...
        public Package(ResourceTable resourceTable) {
            super(resourceTable, TABLE_PACKAGE);
        }
    ...
    public static abstract class Entry {
        ...
        /**
         * Reference into {@link Package#keyStrings} identifying
         * this entry
         */
        public int key;
    ...
    public static abstract class Type extends Component {
        ...
        public Type(ResourceTable resourceTable) {
            super(resourceTable, TABLE_TYPE);
        }
    ...
    public static abstract class TypeSpec extends Component {
        ...
        public TypeSpec(ResourceTable resourceTable) {
            super(resourceTable, TABLE_TYPE_SPEC);
        }
    ...
    public static class Library extends Component {
        ...
        public Library(ResourceTable resourceTable) {
            super(resourceTable, TABLE_LIBRARY);
        }
    ...
    final List<PackageGroup> packageGroups = new ArrayList<PackageGroup>();
    ...
    public ResourceName getResourceName(final int resId) {
        ...
        final Package pkg = group.getPackage(Internal.getPackage(resId));
        final TypeSpec spec = pkg.specs.get(typeIndex);

        for (final Type type : spec.configs) {
            final IndexedEntry<Entry> entry = type.entries.get(entryIndex);
            if (Entry.NO_ENTRY == entry.index || null == entry.value) {
                continue;
            }
    ...
    public Package[] getPackages() {
        ...
        for (PackageGroup group : this.packageGroups) {
    ...
    public void purge(final Symbols symbols, java.util.Map<Integer, Integer> idMap) {
        ...
        for (final PackageGroup pg : this.packageGroups) {
            for (final Package pkg : pg.packages) {
                final List<Integer> retainedKeyIds = new ArrayList<Integer>(); // index of string in key string pool
                final List<TypeSpec> retainedTypeSpecs = findAll(pkg.specs, new Filter<TypeSpec>() {
                    @Override
                ...
                for (final TypeSpec spec : retainedTypeSpecs) {
                    ...
                    for (int i = 0, configCount = spec.configs.size(); i < configCount; i++) {
                        final Type config = spec.configs.get(i);
                        if (config.entries.isEmpty()) {
                            continue;
                        }

                        final List<IndexedEntry<Entry>> retainedEntries = new ArrayList<IndexedEntry<Entry>>();

                        for (int j = 0, entryCount = config.entries.size(); j < entryCount; j++) {
                            final IndexedEntry<Entry> entry = config.entries.get(j);
                            ...
                            if (entry.value instanceof ValueEntry) {
                                final ValueEntry ve = (ValueEntry) entry.value;
                            ...
                            } else if (entry.value instanceof MapEntry) {
                                final MapEntry me = (MapEntry) entry.value;
                                if (idMap.containsKey(me.parent)) {
                                    me.parent = idMap.get(me.parent);
                                }

                                for (final Map map : me.values) {
                ...
                for (final TypeSpec spec : retainedTypeSpecs) {
                    for (final Type config : spec.configs) {
                        for (final IndexedEntry<Entry> entry : config.entries) {
                            if (Entry.NO_ENTRY == entry.index || null == entry.value) {
                                continue;
                            }
                ...
                java.util.Collections.sort(retainedTypeSpecs, new Comparator<TypeSpec>() {
                    @Override
