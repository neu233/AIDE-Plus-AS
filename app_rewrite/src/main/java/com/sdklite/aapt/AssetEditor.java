                ...
                for (int i = 0; i < npkg; i++) {
                    ...
                    while (tell() - p < this.size) {
                        ...
                        case TABLE_TYPE_SPEC: {
                            final TypeSpec spec = parseResourceTableTypeSpec(pkg);
                            pkg.specs.add(spec);
                            break;
                        }
                        case TABLE_TYPE: {
                            final Type type = parseResourceTableType(pkg);
        ...
        return new StringPool() {
            {
                ...
                for (int i = 0; i < styleCount; i++) {
                    this.styles.add(new IndexedEntry<Style>(readInt(), new Style()));
                }
                ...
                for (int i = 0; i < styleCount; i++) {
                    final IndexedEntry<Style> entry = this.styles.get(i);
                    seek(p + this.stylesStart + entry.index);

                    for (long p = tell(); Span.END != readInt(); p = tell()) {
                        seek(p);

                        final Span span = parseStringPoolSpan();
                        entry.value.add(span);
                        if (span.name == Span.END) {
                            break;
                        }
                    }
                }
