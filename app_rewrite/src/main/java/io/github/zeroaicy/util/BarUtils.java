...
public final class BarUtils {
    ...
    public static int getActionBarHeight() {
        ...
        if (ContextUtil.getActivity().getTheme().resolveAttribute(android.R.attr.actionBarSize, tv, true)) {
            return TypedValue.complexToDimensionPixelSize(
                    tv.data, ContextUtil.getActivity().getResources().getDisplayMetrics()
            );
        }
        return 0;
    }
    ...
    private static void invokePanels(final String methodName) {
        try {
            @SuppressLint("WrongConstant")
				Object service = ContextUtil.getContext().getSystemService("statusbar");
    ...
    public static boolean isNavBarVisible(@NonNull final Window window) {
        ...
        if (isVisible) {
            ...
            if (Build.MANUFACTURER.equalsIgnoreCase("samsung")
                ...
                try {
						return Settings.Global.getInt(ContextUtil.getContext().getContentResolver(), "navigationbar_hide_bar_enabled") == 0;
    ...
    private static String getResNameById(int id) {
        try {
            return ContextUtil.getContext().getResources().getResourceEntryName(id);
        } catch (Exception ignore) {
            return "";
        }
    }
    ...
    public static boolean isSupportNavBar() {
        ...
        boolean menu = ViewConfiguration.get(ContextUtil.getContext()).hasPermanentMenuKey();
