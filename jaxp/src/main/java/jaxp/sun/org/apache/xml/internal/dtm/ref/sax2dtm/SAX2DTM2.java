 ...
 */
package jaxp.sun.org.apache.xml.internal.dtm.ref.sax2dtm;
...
import jaxp.sun.org.apache.xml.internal.dtm.DTMException;
...
import jaxp.sun.org.apache.xml.internal.dtm.ref.ExpandedNameTable;
import jaxp.sun.org.apache.xml.internal.dtm.ref.ExtendedType;
import jaxp.sun.org.apache.xml.internal.res.XMLErrorResources;
...
import jaxp.sun.org.apache.xml.internal.utils.FastStringBuffer;
import jaxp.sun.org.apache.xml.internal.utils.SuballocatedIntVector;
import jaxp.sun.org.apache.xml.internal.utils.XMLString;
import jaxp.sun.org.apache.xml.internal.utils.XMLStringDefault;
import jaxp.sun.org.apache.xml.internal.utils.XMLStringFactory;
...
//%HZ%: Added reference to DTMDefaultBase.ROOTNODE back in, temporarily
      ...
      {
        _startNode = node;
        _currentNode = (node == DTM.NULL) ? NULL
...
//%HZ%: Added reference to DTMDefaultBase.ROOTNODE back in, temporarily
    ...
    public int next()
    {
      ...
      if (nodeType != ELEMENT_NODE) {
        while (node != NULL && _exptype2(node) != nodeType) {
          node = _nextsib2(node);
        }
      }
...
//%HZ%: Added reference to DTMDefaultBase.ROOTNODE back in, temporarily
    ...
    public int next()
    {
      _currentNode = (_currentNode == NULL) ? NULL
          ...
          extends FollowingSiblingIterator
  {
    ...
    public int next()
    {
      ...
      if (nodeType != ELEMENT_NODE) {
        while ((node = _nextsib2(node)) != NULL && _exptype2(node) != nodeType) {}
      }
      else {
        while ((node = _nextsib2(node)) != NULL && _exptype2(node) < NTYPES) {}
      }
          ...
          extends PrecedingSiblingIterator
  {
    ...
    public int next()
    {
      ...
      if (nodeType != ELEMENT_NODE) {
        ...
        while (node != NULL && node != startNodeID && _exptype2(node) < NTYPES) {
          node = _nextsib2(node);
        }
      }

      if (node == NULL || node == startNodeID) {
        _currentNode = NULL;
        return NULL;
      }
      else {
      ...
      if (_isRestartable)
      {
        ...
        if (nodeType >= NTYPES) {
          ...
          while (nodeID != END) {
            ...
            if ((eType < NTYPES && eType == nodeType)
                || (eType >= NTYPES
...
//%HZ%: Added reference to DTMDefaultBase.ROOTNODE back in, temporarily
  ...
  public final class TypedDescendantIterator extends DescendantIterator
  {
    ...
    public int next()
    {
      ...
      else if (startNode == ROOTNODE)
      {
        ...
        } while (expType < NTYPES
                || m_extendedTypes[expType].getNodeType() != ELEMENT_NODE);
      }
      else
      {
        do
      ...
      throws SAXException
  {
    ...
    int exName = m_expandedNameTable.getExpandedTypeID(uri, localName, ELEMENT_NODE);

    int prefixIndex = (qName.length() != localName.length())
                      ? m_valuesOrPrefixes.stringToIndex(qName) : 0;

    int elemNode = addNode(ELEMENT_NODE, exName,
    ...
    if(!m_pastFirstElement)
    {
      ...
      exName = m_expandedNameTable.getExpandedTypeID(null, prefix, NAMESPACE_NODE);
      m_values.addElement(declURL);
      int val = m_valueIndex++;
      addNode(NAMESPACE_NODE, exName, elemNode,
        NULL, val, false);
    ...
    for (int i = startDecls; i < nDecls; i += 2)
    {
      ...
      addNode(NAMESPACE_NODE, exName, elemNode, NULL, val, false);
    }
    ...
    for (int i = 0; i < n; i++)
    {
      ...
      addNode(nodeType, exName, elemNode, NULL, val,
                     false);
    }
  ...
  public void comment(char ch[], int start, int length) throws SAXException
  {
    ...
    m_previous = addNode(COMMENT_NODE, COMMENT_NODE,
  ...
  protected final void charactersFlush()
  {
    ...
    if (m_textPendingStart >= 0)  // -1 indicates no-text-in-progress
    {
      ...
      if (doStrip) {
        ...
        if (length > 0) {
          ...
          if (length <= TEXT_LENGTH_MAX
            ...
            m_previous = addNode(m_coalescedTextType, TEXT_NODE,
      ...
      m_textType = m_coalescedTextType = TEXT_NODE;
    }
  }
          ...
          throws SAXException
  {
    ...
    int dataIndex = m_data.size();
    m_previous = addNode(PROCESSING_INSTRUCTION_NODE,
  ...
  public String getLocalName(int nodeHandle)
  {
    ...
    if (expType == PROCESSING_INSTRUCTION_NODE)
    {
  ...
  public final String getNodeNameX(int nodeHandle)
  {
    ...
    if (eType == PROCESSING_INSTRUCTION_NODE)
    {
  ...
  public String getStringValue()
  {
    ...
    if (child == NULL) return EMPTY_STR;

    // optimization: only create StringBuffer if > 1 child
    if ((_exptype2(child) == TEXT_NODE) && (_nextsib2(child) == NULL))
    {
  ...
  public String getNodeValue(int nodeHandle)
  {
    ...
    else if (ELEMENT_NODE == type || DOCUMENT_FRAGMENT_NODE == type
             || DOCUMENT_NODE == type)
    {
      return null;
    }
    ...
    protected final int getNextNamespaceNode2(int baseID) {
        int type;
        while ((type = _type2(++baseID)) == ATTRIBUTE_NODE);
    ...
    protected final void copyAttributes(final int nodeID, SerializationHandler handler)
       ...
       for(int current = getFirstAttributeIdentity(nodeID); current != NULL; current = getNextAttributeIdentity(current)){
