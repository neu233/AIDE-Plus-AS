...
public class Logger {
    ...
    public static void showToast(final Context context, final String toastText, final boolean longDuration) {
        if (context == null)
            return;
    ...
    public static CharSequence[] getLogLevelsArray() {
        return new CharSequence[] { String.valueOf(LOG_LEVEL_OFF), String.valueOf(LOG_LEVEL_NORMAL), String.valueOf(LOG_LEVEL_DEBUG), String.valueOf(LOG_LEVEL_VERBOSE) };
    }
    ...
    public static boolean shouldEnableLoggingForCustomLogLevel(Integer customLogLevel) {
        if (customLogLevel == null || CURRENT_LOG_LEVEL <= LOG_LEVEL_OFF || customLogLevel <= LOG_LEVEL_OFF)
            return false;
