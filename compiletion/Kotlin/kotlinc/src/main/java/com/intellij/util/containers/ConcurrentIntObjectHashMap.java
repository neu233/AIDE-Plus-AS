...
public final class ConcurrentIntObjectHashMap<V> implements ConcurrentIntObjectMap<V> {
    ...
    ConcurrentIntObjectHashMap(int initialCapacity, float loadFactor, int concurrencyLevel) {
        ...
        sizeCtl = (size >= (long) MAXIMUM_CAPACITY) ? MAXIMUM_CAPACITY : tableSizeFor((int) size);
    }
    ...
    public boolean equals(Object o) {
        if (o != this) {
            ...
            for (Entry e : m.entrySet()) {
                ...
                V v;
                if ((v = get(mk)) == null || mv != v && !mv.equals(v)) {
                    return false;
                }
            }
        }
    ...
    private void tryPresize(int size) {
        ...
        while ((sc = sizeCtl) >= 0) {
            ...
            if (tab == null || (n = tab.length) == 0) {
                ...
                int rs = resizeStamp(n);
                if (compareAndSwapInt(this, SIZECTL, sc, (rs << RESIZE_STAMP_SHIFT) + 2)) {
                    transfer(tab, null);
                }
            }
        }
            ...
            T[] r =
                    (a.length >= m)
                            ...
                            java.lang.reflect.Array.newInstance(
                                    Objects.requireNonNull(a.getClass().getComponentType()), m);
            int n = r.length;
            int i = 0;
            for (E e : this) {
                if (i == n) {
