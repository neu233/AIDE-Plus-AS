...
import jaxp.sun.org.apache.xalan.internal.xsltc.compiler.util.NumberType;
...
final class EqualityExpr extends Expression {
    ...
    public void translate(ClassGenerator classGen, MethodGenerator methodGen) {
        ...
        if (_op == Operators.NE) {
                ...
                il.append(InstructionConstants.IXOR);                        // not x <-> x xor 1
            }
            return;
        }
        ...
        if (tleft instanceof NodeSetType && tright instanceof BooleanType) {
            ...
            il.append(InstructionConstants.IXOR); // x != y <-> x xor y
        if (_op == Operators.EQ) {
                il.append(InstructionConstants.ICONST_1);
                il.append(InstructionConstants.IXOR); // not x <-> x xor 1
            }
