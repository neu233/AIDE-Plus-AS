...
import jaxp.sun.org.apache.xml.internal.utils.NodeVector;
...
//%HZ%: Added reference to DTMDefaultBase.ROOTNODE back in, temporarily
    ...
    public int next()
    {
      ...
      } else if (_nodeType != DTM.NULL) {
        if (_nodeType != getNodeType(_currentNode)) {
          result = DTMAxisIterator.END;
        }
      }

      _currentNode = DTMAxisIterator.END;
...
//%HZ%: Added reference to DTMDefaultBase.ROOTNODE back in, temporarily
  ...
  public class TypedNamespaceIterator extends NamespaceIterator
  {
    ...
    public int next()
    {
      ...
      return (_currentNode = DTMAxisIterator.END);
    }
...
//%HZ%: Added reference to DTMDefaultBase.ROOTNODE back in, temporarily
          ...
          extends PrecedingSiblingIterator
  {
    ...
    public int next()
    {
      ...
      if (nodeType >= DTM.NTYPES) {
        while (node != DTM.NULL && node != startID && _exptype(node) != nodeType) {
          node = _nextsib(node);
        }
      } else {
        while (node != DTM.NULL && node != startID) {
    ...
    public int next()
    {
        ...
        // the tail-recursion.
                        ...
                        ++_currentNode)
                {
                        ...
                        {
                                if(_type(_currentNode) != DTM.ATTRIBUTE_NODE &&
                                        _type(_currentNode) != DTM.NAMESPACE_NODE)
                                        return returnNode(makeNodeHandle(_currentNode));
                        }
                        else
                                --_sp;
...
//%HZ%: Added reference to DTMDefaultBase.ROOTNODE back in, temporarily
    ...
    public int next()
    {
      ...
      } while(DTM.ATTRIBUTE_NODE == type || DTM.TEXT_NODE == type
  ...
  public class NthDescendantIterator extends DescendantIterator
  {
    ...
    public int next()
    {
      ...
      while ((node = super.next()) != DTMAxisIterator.END)
      {
        node = makeNodeIdentity(node);
        ...
        while ((pos < _pos) && (child = _nextsib(child)) != DTMAxisIterator.END);
